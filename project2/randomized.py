            ...
            if os.path.exists(file):
                with open(file) as f:
                    ...
                    for i in range(num_simulations):
                        ...
                        already_tested = 0
                        while tuple(solution) in solutions:
                            already_tested += 1
                            if already_tested > 10:
                                break
                    ...
                    #result_file.write(f"{len(v)} \t\t\t{p} \t\t\t\t\t{len(e)} \t\t\t\t\t{min_prev} \t\t\t\t\t\t{operations_number} \t\t\t\t{len(solutions)} \t\t\t\t{end - start}\n")
                    print("Vertices: ", len(v),"%: ", p,"time: ", end - start, "min edge: ", min_prev, "operations: ", operations_number, "solutions: ", len(solutions))